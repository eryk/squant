  ...
  case class Source(sourceType: String = "", config: TaskConfig = TaskConfig(), isInit: Boolean = false) extends Command {
    override def run(): Unit = {
      sourceType match {
        ...
        case "all" => if (isInit) {
          StockBasicsSource.init(config)
          DailyKTypeDataSource.init(config)
          FinanceDataSource.init(config)
          MinuteKTypeDataSource.init(config)
          MoneyFlowDataSource.init(config)
          StockCategoryDataSource.init(config)
          TickDataSource.init(config)
        } else {
